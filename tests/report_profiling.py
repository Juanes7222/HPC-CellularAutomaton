...
Fixes applied (v2):
  [FIX-1] chart_performance: x-axis uses categorical positions (range(len(sizes)))
           instead of raw numeric values, preventing N=8k/64k overlap on a linear
           axis that spans up to 2,000,000.  Same fix applied to chart_velocity,
           chart_miss_rates, and chart_counters.
  [FIX-2] chart_performance (heap panel): peak_heap_mb values of 0.0 are replaced
           with NaN before plotting so that skipped Massif runs (N > VALGRIND_MAX_N)
           do not draw a misleading drop to zero.
  [FIX-3] chart_performance (throughput panel): y-axis lower limit is set to
           max(0, min(values)*0.995) so the ~1% variation is shown in context
           rather than appearing as a huge oscillation.
  [FIX-4] perf_extract (Python): keyword matching now requires a word boundary
           after the keyword to prevent "instructions" matching "branch-instructions".
...
# [FIX-4] Word-boundary anchor prevents "instructions" matching "branch-instructions".
...
def normalize_dataframe(path: str) -> pd.DataFrame:
    ...
    # [FIX-2] Replace 0.0 heap values with NaN so skipped Massif runs
    # (N > VALGRIND_MAX_N) do not plot as a misleading drop to zero.
...
def parse_massif(path: str) -> dict[str, Any]:
    ...
    # Sentinel written by bench_profiling.sh when N > VALGRIND_MAX_N.
...
# [FIX-1] Helper: returns (x_positions, x_labels) for consistent categorical
# x-axis across all chart functions.  Avoids linear-scale crowding of
# values like [8000, 64000, 500000, 2000000] on a single axis.
...
def chart_performance(df: pd.DataFrame) -> str:
    ...
    # [FIX-1] Use categorical positions instead of raw numeric values.
    ...
    # -- Throughput --
    # [FIX-3] Set y-axis lower bound to show real variation (~1%) in context.
    ...
    # -- Heap memory --
    # [FIX-2] NaN values (skipped Massif runs) produce a gap in the line
    # instead of a drop to zero.
...
def chart_velocity(df: pd.DataFrame) -> str:
    sizes = df["road_length"].astype(int).tolist()
    pos, labels = _cat_axis(sizes)  # [FIX-1]
...
def chart_counters(df: pd.DataFrame) -> str:
    ...
    if "llc_miss_pct" in df.columns and df["llc_miss_pct"].notna().any():
        axs[1, 1].plot(pos, df["llc_miss_pct"], marker="o", lw=2.3, color="#70AD47")
        annotate_points(axs[1, 1], pos, df["llc_miss_pct"], d=2)
...
def _write_overview(wb: Workbook, df: pd.DataFrame, raw: dict[int, dict[str, Any]]) -> None:
    ...
    if timing.get("mean_vel") is not None and not math.isnan(_sf(timing.get("mean_vel"))):
        rows_data.append(("Velocidad media (ρ=0.50)", f"N={largest_n:,}",
                          ...
                          "Coherencia entre repeticiones valida estado estacionario."))
    if gprof_top:
    ...
    if "llc_miss_pct" in df.columns and df["llc_miss_pct"].notna().any():
        ...
        rows_data.append(("LLC miss rate máximo", f"N={int(row_max['road_length']):,}",
                          ...
                          "Cuando N supera la LLC el cuello de botella cambia a ancho de banda DRAM."))
    ...
    r += 1
    _title_row(ws, "Per-size quick view", 7, row=r); r += 1
    ...
    for _, row in df.iterrows():
        bg   = "alt" if r % 2 == 0 else "white"
        vals = [int(row["road_length"]),
                _sf(row.get("time_mean_ms")), _sf(row.get("time_std_ms")),
                _sf(row.get("throughput_mcells_s")), _sf(row.get("ipc")),
                _sf(row.get("llc_miss_pct")),   _sf(row.get("peak_heap_mb"))]
        fmts = ["#,##0", "0.000", "0.000", "0.000000", "0.0000", "0.00", "0.000"]
        for ci, (v, f) in enumerate(zip(vals, fmts), 1):
            _dcell(ws.cell(r, ci), v, fmt=f, bg=bg,
        ...
        for ci, col in enumerate(df.columns, 1):
            ...
            if col == "road_length":
                ...
                fmt = ("0.00" if "pct" in col
                       else "#,##0" if any(k in col for k in
                                           ["instructions", "cycles", "refs", "misses"])
                       else "0.000")
...
def _write_raw_summary(wb: Workbook, df: pd.DataFrame,
                       raw: dict[int, dict[str, Any]]) -> None:
...
def _write_hot_lines(wb: Workbook, df: pd.DataFrame,
                     raw: dict[int, dict[str, Any]]) -> None:
    ...
    for n in df["road_length"].astype(int).tolist():
        ...
        if not hot:
            ...
            _dcell(ws.cell(r, 4), "Sin líneas anotadas (N > VALGRIND_MAX_N)",
                   bg=bg, align="left")
            r += 1; continue
        for item in hot:
            bg = "alt" if r % 2 == 0 else "white"
...
def _write_analysis_notes(wb: Workbook, df: pd.DataFrame,
                          raw: dict[int, dict[str, Any]]) -> None:
    ...
    notes = [
        ("Modelo de acceso", "Autómata 1D",
         "El bucle principal accede a cells[] y next_cells[] de forma estrictamente "
         "secuencial. El patrón es ideal para el prefetcher de hardware: L1 miss rate "
         "esperada < 2%."),
        ...
        ("Paralelizabilidad", "compute_next_generation",
         "Las iteraciones son completamente independientes (cells[] solo lectura, "
         "next_cells[] solo escritura). La ausencia de dependencias de datos hace "
         "el bucle apto para #pragma omp parallel for."),
        ("IPC", f"N={largest_n:,}",
         f"IPC={_fn(last.get('ipc'), 4)} → {_classify_ipc(_sf(last.get('ipc')))}. "
         "El kernel es aritméticamente simple (2 operaciones lógicas + 3 lecturas + "
         "1 escritura por celda). IPC bajo indicaría espera por datos de memoria, "
         "no por operaciones aritméticas."),
        ("LLC miss rate", f"N={largest_n:,}",
         f"LLC miss={_fp(last.get('llc_miss_pct'))} → "
         f"{_classify_miss(_sf(last.get('llc_miss_pct')))}. "
         "Para N donde el working set supera la LLC, este valor sube e indica que "
         "el speedup OpenMP estará limitado por el ancho de banda del bus de memoria."),
    ]

    if timing.get("count", 0):
        notes.append(("Estabilidad", f"N={largest_n:,}",
                       f"Media={_fn(timing.get('mean_ms'), 3)} ms, "
                       f"std={_fn(timing.get('std_ms'), 3)} ms, "
                       f"CV={_fp(timing.get('cv_pct'))}. CV < 5% es criterio de "
                       "medición válida."))
    if timing.get("mean_vel") and not math.isnan(_sf(timing.get("mean_vel"))):
        notes.append(("Velocidad asintótica", "ρ=0.50",
                       f"v̄={_fn(timing['mean_vel'], 4)} ± {_fn(timing['std_vel'], 4)}. "
                       "El modelo predice v̄ ≈ 0.5 para ρ=0.5 en el punto de transición "
                       "de fase. Consistencia entre repeticiones confirma estado "
                       "estacionario alcanzado."))
    if gprof_top:
        notes.append(("gprof hotspot", f"N={largest_n:,}",
                       f"{gprof_top[0]['name']} concentra "
                       f"{_fp(gprof_top[0]['pct_time'])} del tiempo muestral. "
                       "Es la función objetivo para la directiva OpenMP."))
    if perf_top:
        notes.append(("perf record", f"N={largest_n:,}",
                       f"Símbolo más pesado: {perf_top[0]['symbol']} con "
                       f"{_fp(perf_top[0]['overhead_pct'])} de overhead."))
    if cache_top:
        notes.append(("cachegrind", f"N={largest_n:,}",
                       f"Por conteo de instrucciones: {cache_top[0]['name']} con "
                       f"{_fp(cache_top[0]['pct'])}. Coherente con gprof."))
    if massif and not math.isnan(_sf(massif.get("peak_total_mb"))):
        notes.append(("Massif", f"N={largest_n:,}",
                       f"Peak heap={_fn(massif.get('peak_total_mb'), 3)} MB; "
                       f"útil={_fp(massif.get('peak_useful_pct'))}. "
                       f"Teórico = {_fn(theoretical_heap_mb(largest_n), 3)} MB "
                       "(2×N×4 B). Ratio ≈ 100% confirma sin sobreasignación."))
...
def build_workbook(df: pd.DataFrame, raw: dict[int, dict[str, Any]],
...
def build_latex_table(df: pd.DataFrame) -> None:
    lines = [
        r"\begin{table}[h]",
        r"\centering",
        r"\caption{Caracterización de la implementación secuencial del autómata "
        r"de tráfico ($\rho=0.50$)}",
    ...
    for _, row in df.iterrows():
        heap_val = _sf(row.get("peak_heap_mb"))
        heap_str = f"{heap_val:.3f}" if not math.isnan(heap_val) else "—"
        lines.append(
...
def build_html_report(df: pd.DataFrame, raw: dict[int, dict[str, Any]],
                      chart_paths: list[str]) -> None:
    import base64
    ...
    def htable(title, rows, nk, vk):
        ...
        return (f'<h3>{html.escape(title)}</h3>'
                f'<table><tr><th>Función / Símbolo</th><th>Share</th></tr>'
                f'{body}</table>')
    ...
    massif_mb  = massif.get("peak_total_mb", math.nan) if massif else math.nan
    ...
    cards_html = "".join([
        ...
        kpi("Peak heap", f"{_fn(massif_mb, 3)} MB",
            f"useful={_fp(massif.get('peak_useful_pct') if massif else math.nan)}"),
    ])
    ...
    charts_html = "\n".join(img_tag(p) for p in chart_paths)
...
<div class="container">
  <div class="hero">
    ...
    <p>Tamaños analizados: {html.escape(sizes_str)} &nbsp;·&nbsp; ρ = 0.50</p>
    <span class="badge">gprof</span>
    <span class="badge">perf stat / record</span>
    <span class="badge">valgrind massif</span>
  ...
  <div class="section card">
    ...
    <table>
      <tr><th>N</th><th>t̄ (ms)</th><th>σ (ms)</th><th>Mcells/s</th>
          <th>IPC</th><th>L1 miss %</th><th>LLC miss %</th><th>Heap (MB)</th></tr>
      {rows_html}
    </table>
  </div>

  <div class="section">
    <h2>Hotspots — N={largest_n:,}</h2>
    <div class="grid two">
      <div class="card">{htable("gprof — top funciones", gprof_top, "name", "pct_time")}</div>
      <div class="card">{htable("perf record — top símbolos", perf_top, "symbol", "overhead_pct")}</div>
      <div class="card">{htable("cachegrind — instruction share", cache_top, "name", "pct")}</div>
    </div>
  </div>
...
def main() -> None:
    ...
    print(f"    {len(df)} rows, N = {df['road_length'].tolist()}")
